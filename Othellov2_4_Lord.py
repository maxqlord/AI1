...
class Othello():
    ...
    def choose(self, player, board, firstPlayer):
        
        a = -1
        choices = []
        boardCopy = board[:]
        for move in self.legal_moves(player, board):
            #boardcopy is original
            self.make_move(move, player, board)
           
            val = self.minimax(self.opponent(player), list(board), firstPlayer)
    ...
    def minimax(self, player, board, firstPlayer):
